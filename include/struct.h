 ...
 * 
 *   $Id: struct.h,v 1.1.1.1.2.16 2004/07/03 19:04:17 syzop Exp $
 */
...
#endif
#define FLAGS_LPOSTFIX   0x20000000 /* User has local postfix */
#define FLAGS_DCCBLOCK   0x40000000 /* Block all DCC send requests */
#define FLAGS_MAP        0x80000000	/* Show this entry in /map */

// client->flags2 (32 bits)
#define FLAGS2_NOIDENT 0x00000001
#define FLAGS2_FROZEN 0x00000002
#define FLAGS2_SUSER 0x00000004
#define FLAGS2_EXCEPT_K 0x00000008
#define FLAGS2_EXCEPT_G 0x00000010

#define ClientNoIdent(x) ((x)->flags2 & FLAGS2_NOIDENT)
#define ClientFrozen(x) ((x)->flags2 & FLAGS2_FROZEN)
#define ClientSUser(x) ((x)->flags2 & FLAGS2_SUSER)
#define ClientExceptK(x) ((x)->flags2 & FLAGS2_EXCEPT_K)
#define ClientExceptG(x) ((x)->flags2 & FLAGS2_EXCEPT_G)

#define SetClientNoIdent(x) ((x)->flags2 |= FLAGS2_NOIDENT)
#define SetClientFrozen(x) ((x)->flags2 |= FLAGS2_FROZEN)
#define SetClientSUser(x) ((x)->flags2 |= FLAGS2_SUSER)
#define SetClientExceptK(x) ((x)->flags2 |= FLAGS2_EXCEPT_K)
#define SetClientExceptG(x) ((x)->flags2 |= FLAGS2_EXCEPT_G)

#define ClearClientFrozen(x) ((x)->flags2 &= ~FLAGS2_FROZEN)
...
#define IsTotalInvis(x)	((x)->umodes & UMODE_TOTALINVIS)
#define IsModeWalk(x)	((x)->umodes & UMODE_MODEWALK)
#define IsCallerID(x)	((x)->umodes & UMODE_CALLERID)
#define IsServOp(x)		((x)->umodes & UMODE_SERVOP)
...
#define SendFailops(x)		(IsPerson(x) && ((x)->user->snomask & SNO_FAILOP))
...
#define	SendWallops(x)		(!IsMe(x) && IsPerson(x))
...
#define IsMobile(x)		((x)->flags & FLAGS_MOBILE)
...
#define ClearServOp(x)      ((x)->umodes &= ~UMODE_SERVOP)
#define ClearNoFakeLag(x)   ((x)->umodes &= ~UMODE_NOFAKELAG)
...
#define ListenerMobile(x)	((x)->listener->umodes & LISTENER_MOBILE)
...
#define OFLAG_ZLINE			0x00040000	/* Oper can use /zline and /unzline */
#define OFLAG_NETADMIN		0x00080000	/* netadmin gets +N */
#define OFLAG_SADMIN		0x00100000	/* services admin gets +a */
#define OFLAG_WHOIS			0x00200000	/* gets auto +W on oper up */
#define OFLAG_HIDE			0x00400000	/* gets auto +x on oper up */
#define OFLAG_TKL			0x00800000	/* can use G:lines and shuns */
#define OFLAG_GZL			0x01000000	/* can use global Z:lines */
#define OFLAG_OVERRIDE		0x02000000	/* can use oper-override */
#define OFLAG_UMODEQ		0x04000000	/* can set +q */
#define OFLAG_NOFAKELAG		0x08000000	/* Can set +f */
#define OFLAG_NOCHANLIMIT	0x10000000	/* Can join unlimited channels */
#define OFLAG_LOCAL			(OFLAG_REHASH|OFLAG_HELPOP|OFLAG_GLOBOP|OFLAG_LOCOP|OFLAG_LROUTE|OFLAG_LKILL|OFLAG_KLINE|OFLAG_UNKLINE|OFLAG_LNOTICE|OFLAG_NOFAKELAG|OFLAG_NOCHANLIMIT)
#define OFLAG_GLOBAL		(OFLAG_LOCAL|OFLAG_WALLOP|OFLAG_GROUTE|OFLAG_GKILL|OFLAG_GNOTICE)
...
#define OPIsNoChanLimit(x)	((x)->oflag & OFLAG_NOCHANLIMIT)
...
#define ACCEPT_LINK_USER 0
#define ACCEPT_LINK_MASK 1
#define ACCEPT_LINK_REF 2

#define AcceptIsUser(x) ((x)->flags == ACCEPT_LINK_USER)
#define AcceptIsMask(x) ((x)->flags == ACCEPT_LINK_MASK)
#define AcceptIsRef(x) ((x)->flags == ACCEPT_LINK_REF)
...
struct User {
	...
	Link *accept;		/* chain of allow pointer blocks */
	u_int32_t last_req;	/* last request (callerid mode) */
...
#define TKL_SPAMF 0x0010
#define TKL_NICK 0x0020
#define TKL_NOIDENT 0x0040
...
#define LISTENER_MOBILE		0x000008
#define LISTENER_SSL		0x000010
#define LISTENER_BOUND		0x000020
#define LISTENER_CP_WIN		0x000040
#define LISTENER_CP_KOI		0x000080
#define LISTENER_CP_MAC		0x000100
#define LISTENER_CP_UTF		0x000200
#define LISTENER_CP_DOS		0x000400
...
typedef struct {
	unsigned long bytes_total; // Суммарный трафик
	unsigned long bytes_prev; // Предыдущее показание счетчика
	time_t bytes_prev_time; // Время снятия предыдущего показания
	time_t exceed_time[3]; // Время последних 3 превышений лимита
} traffic_counter;
...
struct Client {
	...
	int codepage;
	traffic_counter tc_recv, tc_send; // Для ограничителя траффика
	int block_transfer; // Заблокировать передачу данных (превышен лимит траффика)
	int requestid;
	u_int32_t idreq_servicestamp;
};
...
struct _configitem_class {
	...
	int sendmaxbps, recvmaxbps, minlastseen;
...
#define	isvalid(c) (((c) >= 'A' && (c) <= '~') || isdigit(c) || (c) == '-' || isvalidr(c))
#ifdef RUSSIAN_NICK
#define isvalidr(c) (((c) >= '\xC0' && (c) <= '\xFF') || (c) == '\xA8' || (c) == '\xB8')
#else
#define isvalidr(c) 0
#endif
