...
int  R_cp_win, R_cp_koi, R_cp_mac, R_cp_utf, R_cp_dos, R_mobile;

char REPORT_DO_DNS[256], REPORT_FIN_DNS[256], REPORT_FIN_DNSC[256],
    REPORT_FAIL_DNS[256], REPORT_DO_ID[256], REPORT_FIN_ID[256],
    REPORT_FAIL_ID[256];
char REPORT_CP_WIN[128], REPORT_CP_KOI[128], REPORT_CP_MAC[128],
    REPORT_CP_UTF[128], REPORT_CP_DOS[128];
char REPORT_MOBILE[128];
...
static TS try_connections(TS currenttime)
{
	...
	for (aconf = conf_link; aconf; aconf = (ConfigItem_link *) aconf->next) {
		...
		if ((aconf->lastseen + cltmp->minlastseen) > aconf->hold) {
			aconf->hold = aconf->lastseen + cltmp->minlastseen;
		}
		...
		if (!cptr && (cltmp->clients < cltmp->maxclients)) {
			...
			if (deny_link) {
				// Линковка запрещена конфигурацией - перейти к
				// следующему серверу
				continue;
			}
...
*/
	...
	(void)ircsprintf(REPORT_CP_WIN, ":%s %s", me.name, BREPORT_CP_WIN);
	(void)ircsprintf(REPORT_CP_KOI, ":%s %s", me.name, BREPORT_CP_KOI);
	(void)ircsprintf(REPORT_CP_MAC, ":%s %s", me.name, BREPORT_CP_MAC);
	(void)ircsprintf(REPORT_CP_UTF, ":%s %s", me.name, BREPORT_CP_UTF);
	(void)ircsprintf(REPORT_CP_DOS, ":%s %s", me.name, BREPORT_CP_DOS);
	(void)ircsprintf(REPORT_MOBILE, ":%s %s", me.name, BREPORT_MOBILE);
	...
	R_cp_win = strlen(REPORT_CP_WIN);
	R_cp_koi = strlen(REPORT_CP_KOI);
	R_cp_mac = strlen(REPORT_CP_MAC);
	R_cp_utf = strlen(REPORT_CP_UTF);
	R_cp_dos = strlen(REPORT_CP_DOS);
	R_mobile = strlen(REPORT_MOBILE);
