...
void vsendto_prefix_one(struct Client *to, struct Client *from, const int fullhost,
    ...
    char *pattern, ...)
{
	...
	for (lp = chptr->members; lp; lp = lp->next)
	{
		...
		if (acptr->from == one || (IsDeaf(acptr) && (!sendanyways || !IsBot(acptr))))
			continue;
		if (MyConnect(acptr))	/* (It is always a client) */
			vsendto_prefix_one(acptr, from, 1, pattern, vl);
    ...
    char *cmd, char *tok, char *nick, char *text, char do_send_check)
{
	...
	int  i;
	char is_ctcp = 0;
	unsigned int tlen, clen, xlen, mlen, wlen = 0;
	char *p;
	...
	/* For mobile users... */
	p = ircsprintf(mcmd, ":%s %s %s :%s", from->name, cmd, nick, text);
	mlen = (int)(p - mcmd);
	ADD_CRLF(mcmd, mlen);
	...
	for (lp = chptr->members; lp; lp = lp->next)
	{
		...
		good:
		i = acptr->from->slot;
		if (IsDeaf(acptr) && (!sendanyways || !IsBot(acptr)))
			continue;
		if (MyConnect(acptr) && IsRegisteredUser(acptr))
		{
			...
			if (IsMobile(acptr) && !IsULine(from)) {
				sendbufto_one(acptr, mcmd, mlen);
			} else if (IsWebTV(acptr) && wlen) {
		...
		for (channels = user->user->channel; channels; channels = channels->next)
			for (users = channels->chptr->members; users; users = users->next)
			{
				...
				if ((channels->chptr->mode.mode & MODE_AUDITORIUM) &&
					...
					continue;
				if (IsTotalInvis(user) && (cptr != user) && !IsAnOper(cptr)) {
					continue;
				}
				sentalong[cptr->slot] = sentalong_marker;
				va_start(vl, pattern);
				vsendto_prefix_one(cptr, user, 1, pattern, vl);
				va_end(vl);
			}
	if (MyConnect(user))
	{
		va_start(vl, pattern);
		vsendto_prefix_one(user, user, 1, pattern, vl);
	}
	va_end(vl);
	return;
}

void sendto_common_channels_normalquit(aClient *user, char *comment)
{
	Membership *channels;
	Member *users;
	aClient *cptr;
	...
	if (user->user)
		for (channels = user->user->channel; channels; channels = channels->next)
			for (users = channels->chptr->members; users; users = users->next)
			{
				...
				sentalong[cptr->slot] = sentalong_marker;
				if (!IsMobile(cptr)) {
					sendto_prefix_one(cptr, user, 1, ":%s QUIT :%s", user->name, comment);
				} else {
					sendto_prefix_one(cptr, user, 1, ":%s QUIT :Quit: +E", user->name);
				}
			}
	if (MyConnect(user))
	{
		if (!IsMobile(user)) {
			sendto_prefix_one(user, user, 1, ":%s QUIT :%s", user->name, comment);
		} else {
			sendto_prefix_one(user, user, 1, ":%s QUIT :Quit: +E", user->name);
		}
	}
	return;
}
    ...
    char *pattern, ...)
{
	...
	for (i = 0; i <= LastSlot; i++)
	{
		...
		/* my client, does he match ? */
			...
			continue;
		va_start(vl, pattern);
		vsendto_prefix_one(cptr, from, 1, pattern, vl);
		va_end(vl);
	}
	va_end(vl);
	return;
...
void sendto_ops_butone(aClient *one, aClient *from, char *pattern, ...)
{
	...
	for (cptr = client; cptr; cptr = cptr->next)
	{
		...
		sentalong[i] = sentalong_marker;
		va_start(vl, pattern);
		vsendto_prefix_one(cptr->from, from, 1, pattern, vl);
		va_end(vl);
	}
	va_end(vl);
	return;
...
void sendto_prefix_one(aClient *to, aClient *from, const int fullhost, const char *pattern, ...)
{
	va_list vl;
	va_start(vl, pattern);
	vsendto_prefix_one(to, from, fullhost, pattern, vl);
	va_end(vl);
}
			...
			char *cmd, char *nick, char *msg)
{
	...
	if (IsULine(from)) {
		// Show full host for all
        	sendto_prefix_one(to, from, 1, ":%s %s %s :%s", sender, cmd, nick, msg);
	} else {
		// Only nick for mobile users
		sendto_prefix_one(to, from, 0, ":%s %s %s :%s", sender, cmd, nick, msg);
	}
}
...
void sendto_channels_inviso_join(aClient *user) {
	...
	if (user->user) {
		for (channels = user->user->channel; channels; channels = channels->next) {
			channels->chptr->users_visible++;
			for (users = channels->chptr->members; users; users = users->next) {
				cptr = users->cptr;
				if (!MyConnect(cptr) || IsAnOper(cptr) || cptr == user) {
					continue;
				}
				if ((channels->chptr->mode.mode & MODE_AUDITORIUM) && !is_chanownprotop(user, channels->chptr) && !(users->flags & (CHFL_CHANOP|CHFL_CHANOWNER|CHFL_CHANPROT))) {
					continue;
				}
				sendto_one(cptr, ":%s!%s@%s JOIN :%s", user->name, user->user->username, GetHost(user), channels->chptr->chname);
			}
		}
	}
	return;
	...
	if (user->user) {
		for (channels = user->user->channel; channels; channels = channels->next) {
			...
			for (users = channels->chptr->members; users; users = users->next) {
				...
				sendto_one(cptr, ":%s!%s@%s PART %s", user->name, user->user->username, GetHost(user), channels->chptr->chname);
			}
		}
	}
	return;
...
void sendto_channel_ntopers(aClient *from, aChannel *chptr, char *pattern, ...) {
	Member *lp;
	aClient *acptr;
	int i;
	++sentalong_marker;
	for (lp = chptr->members; lp; lp = lp->next) {
		acptr = lp->cptr;
		if (acptr->from == from || !IsAnOper(acptr) || (IsDeaf(acptr) && (!sendanyways || !IsBot(acptr)))) {
			continue;
		}
		if (MyConnect(acptr)) { /* (It is always a client) */
			va_list vl;
...
void sendto_chanops_ntopers(aClient *from, aChannel *chptr, char *pattern, ...) {
	va_list vl;
	Member *lp;
	aClient *acptr;
	int i;
	...
	for (lp = chptr->members; lp; lp = lp->next) {
		acptr = lp->cptr;
		if (acptr->from == from || !IsAnOper(acptr) || (IsDeaf(acptr) && (!sendanyways || !IsBot(acptr))) || !(lp->flags & (CHFL_CHANOP|CHFL_CHANOWNER|CHFL_CHANPROT))) {
			continue;
		}
		if (MyConnect(acptr)) { /* (It is always a client) */
			vsendto_prefix_one(acptr, from, 1, pattern, vl);
