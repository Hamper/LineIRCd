...
void send_part_to_local_users (aClient *sptr, aChannel *chptr, char *comment, int quit);
...
Ban *is_banned(aClient *sptr, aChannel *chptr, int type)
{
	...
	if (MyConnect(sptr)) {
		mine = 1;
		s = make_nick_user_host(sptr->name, sptr->user->username, sptr->user->ip_str);
...
CMD_FUNC(m_mode)
{
	    ...
	    && (*parv[2] == '+' || *parv[2] == '-'))))
	{
			...
			return 0;
		{
			...
			{
				if (is_chanowner(member->cptr, chptr) && !(IsTotalInvis(member->cptr) && (member->cptr != sptr) && !IsAnOper(sptr)))
	    ...
	    && (*parv[2] == '+' || *parv[2] == '-'))))
	{
			...
			return 0;
		{
			...
			for (member = chptr->members; member; member = member->next)
			{
				if (is_chanprot(member->cptr, chptr) && !(IsTotalInvis(member->cptr) && (member->cptr != sptr) && !IsAnOper(sptr)))
...
/* do pro-opping here (popping) */
	  ...
	  case MODE_VOICE:
		  ...
		  /* +I Patch */
		  if (IsTotalInvis(who) && (who != cptr) && !IsAnOper(cptr) && !IsULine(cptr) && MyClient(cptr)) {
			  sendto_one(cptr, err_str(ERR_USERNOTINCHANNEL),
			      me.name, cptr->name, who->name, chptr->chname);
			  break;
		  }
...
int can_join(aClient *cptr, aClient *sptr, aChannel *chptr, char *key, char *link, char *parv[], int *no_modewalk)
{
Link *lp;
Ban *banned;
	*no_modewalk = 0;
	...
	*no_modewalk = 0;
	...
	for (lp = sptr->user->invited; lp; lp = lp->next)
        ...
        if ((chptr->mode.limit && chptr->users_visible >= chptr->mode.limit))
...
void join_channel(aChannel *chptr, aClient *cptr, aClient *sptr, int flags)
{
	 ...
	 */
	if (IsTotalInvis(sptr)) {
		sendto_snomask(SNO_EYES, "*** Invisibility -- %s (%s@%s) JOIN %s", sptr->name, sptr->user->username, sptr->user->realhost, chptr->chname);
		ircd_log(LOG_OVERRIDE,"INVISIBILITY: %s (%s@%s) JOIN %s", sptr->name, sptr->user->username, sptr->user->realhost, chptr->chname);
		if (MyClient(sptr)) {
			sendto_one(sptr, ":%s!%s@%s JOIN :%s", sptr->name, sptr->user->username, GetHost(sptr), chptr->chname);
		}
		if (chptr->mode.mode & MODE_AUDITORIUM) {
			sendto_chanops_ntopers(sptr, chptr,  ":%s JOIN :%s", sptr->name, chptr->chname);
		} else {
			sendto_channel_ntopers(sptr, chptr,  ":%s JOIN :%s", sptr->name, chptr->chname);
		}
	} else {
		if (chptr->mode.mode & MODE_AUDITORIUM) {
			if (MyClient(sptr)) {
			...
			sendto_chanops_butone(NULL, chptr, ":%s!%s@%s JOIN :%s", sptr->name, sptr->user->username, GetHost(sptr), chptr->chname);
		} else {
	...
	*jbuf = '\0';
	    ...
	    name = strtoken(&p, NULL, ","))
	{
		...
		if (*name == '0' && !atoi(name))
		{
			...
			{
				chptr = lp->chptr;
				send_part_to_local_users(sptr, chptr, "Left all channels", 0);
				on_channels = 1;
				if (MyConnect(sptr))
					RunHook4(HOOKTYPE_LOCAL_PART, cptr, sptr, chptr, "Left all channels");
				remove_user_from_channel(sptr, chptr);
			}
			if (IsTotalInvis(sptr) && on_channels) {
				sendto_snomask(SNO_EYES, "*** Invisibility -- %s (%s@%s) JOIN 0 (Left all channels)", sptr->name, sptr->user->username, sptr->user->realhost);
				ircd_log(LOG_OVERRIDE,"INVISIBILITY: %s (%s@%s) JOIN 0 (Left all channels)", sptr->name, sptr->user->username, sptr->user->realhost, chptr->chname);
			}
		...
		if (MyConnect(sptr))
		{
			...
			if (!OPIsNoChanLimit(sptr)) {	/* opers can join unlimited chans */
				...
				if (IsHelpOp(sptr)) {
					maxchannels += HELPOP_CHANLIMIT;
				}
				if (sptr->user->joined >= maxchannels)
				{
					sendto_one(sptr,
					    err_str
					    (ERR_TOOMANYCHANNELS),
...
/* RESTRICTCHAN */
		...
		else
		{
			...
			if (i != HOOK_ALLOW && 
			   (i = can_join(cptr, sptr, chptr, key, link, parv, &no_modewalk)))
			{
				if (IsModeWalk(sptr) && !no_modewalk) {
					sendto_snomask(SNO_EYES, "*** OperOverride (ModeWalk) -- %s (%s@%s) JOIN %s", sptr->name, sptr->user->username, sptr->user->realhost, chptr->chname);
					sendto_serv_butone_token(NULL, me.name, MSG_SENDSNO, TOK_SENDSNO, "e :*** OperOverride (ModeWalk) -- %s (%s@%s) JOIN %s", sptr->name, sptr->user->username, sptr->user->realhost, chptr->chname);
					ircd_log(LOG_OVERRIDE,"OVERRIDE (LOCAL MODEWALK): %s (%s@%s) JOIN %s.", sptr->name, sptr->user->username, sptr->user->realhost, chptr->chname);
				} else {
					if (i != -1)
...
CMD_FUNC(m_part)
{
	...
	for (; (name = strtoken(&p, parv[1], ",")); parv[1] = NULL)
	{
		...
		if (IsTotalInvis(sptr)) {
			sendto_snomask(SNO_EYES, "*** Invisibility -- %s (%s@%s) PART %s", sptr->name, sptr->user->username, sptr->user->realhost, chptr->chname);
			ircd_log(LOG_OVERRIDE,"INVISIBILITY: %s (%s@%s) PART %s", sptr->name, sptr->user->username, sptr->user->realhost, chptr->chname);
		}

		/* Send to local users... */
		send_part_to_local_users(sptr, chptr, comment, 0);
	...
	for (hashnum = lopt->starthash; hashnum < CH_MAX; hashnum++)
	{
		if (numsend > 0)
			    ...
			    chptr; chptr = chptr->hnextch)
			{
				...
				if ((chptr->users_visible == 0) && !IsAnOper(cptr))
					continue;
				...
				if ((!lopt->showall))
				{
					...
					if (((IsAnOper(cptr) ? chptr->users : chptr->users_visible) < lopt->usermin) || 
					    ((lopt->usermax >= 0) && ((IsAnOper(cptr) ? chptr->users : chptr->users_visible) > 
					    lopt->usermax)))
						continue;
...
CMD_FUNC(m_names)
{
	...
	for (cm = chptr->members; cm; cm = cm->next)
	{
		...
		if (IsInvisible(acptr) && !member && !IsNetAdmin(sptr))
			continue;
		if (IsTotalInvis(acptr) && (acptr != sptr) && !IsAnOper(sptr))
			continue;
		if (chptr->mode.mode & MODE_AUDITORIUM)
			if (!is_chan_op(sptr, chptr)
...
char *comment = "Rejoining because of user@host change";
	for (tmp = sptr->user->channel; tmp; tmp = tmp->next)
	{
		...
		{
			sendto_chanops_butone(sptr, chptr, ":%s!%s@%s PART %s :Part: %s", sptr->name, sptr->user->username, GetHost(sptr), chptr->chname, comment);
		} else {
			sendto_channel_butserv_butone(chptr, sptr, sptr, ":%s PART %s :Part: %s", sptr->name, chptr->chname, comment);
		}
	}
}
...
void send_part_to_local_users (aClient *sptr, aChannel *chptr, char *comment, int quit) {
	/* Send to user */
	if (MyClient(sptr) && !quit) {
		if (!(chptr->mode.extmode & EXTMODE_NOQUIT)) {
			if (!comment) {
				sendto_one(sptr, ":%s!%s@%s PART %s", sptr->name, sptr->user->username, GetHost(sptr), chptr->chname);
			} else {
				sendto_one(sptr, ":%s!%s@%s PART %s :Part: %s", sptr->name, sptr->user->username, GetHost(sptr), chptr->chname, comment);
			}
		} else {
			if (!comment) {
				sendto_one(sptr, ":%s!%s@%s PART %s", sptr->name, sptr->user->username, GetHost(sptr), chptr->chname);
			} else {
				sendto_one(sptr, ":%s!%s@%s PART %s :Part: +P", sptr->name, sptr->user->username, GetHost(sptr), chptr->chname);
			}
		}
	}
	/* Send to other users */
	if (IsTotalInvis(sptr)) {
		if ((chptr->mode.mode & MODE_AUDITORIUM) && !is_chanownprotop(sptr, chptr)) {
			if (!(chptr->mode.extmode & EXTMODE_NOQUIT)) {
				if (!comment) {
					sendto_chanops_ntopers(sptr, chptr, ":%s PART %s", sptr->name, chptr->chname);
				} else {
					sendto_chanops_ntopers(sptr, chptr, ":%s PART %s :Part: %s", sptr->name, chptr->chname, comment);
				}
			} else {
				if (!quit) {
					if (!comment) {
						...
						sendto_chanops_ntopers(sptr, chptr, ":%s PART %s :Part: +P", sptr->name, chptr->chname);
					}
				} else {
					sendto_chanops_ntopers(sptr, chptr, ":%s PART %s :Quit: +P", sptr->name, chptr->chname);
				}
			}
		} else {
			if (!(chptr->mode.extmode & EXTMODE_NOQUIT)) {
				if (!comment) {
					sendto_channel_ntopers(sptr, chptr, ":%s PART %s", sptr->name, chptr->chname);
				} else {
					sendto_channel_ntopers(sptr, chptr, ":%s PART %s :Part: %s", sptr->name, chptr->chname, comment);
				}
			} else {
				if (!quit) {
					if (!comment) {
						...
						sendto_channel_ntopers(sptr, chptr, ":%s PART %s :Part: +P", sptr->name, chptr->chname);
					}
				} else {
					sendto_channel_ntopers(sptr, chptr, ":%s PART %s :Quit: +P", sptr->name, chptr->chname);
				}
			}
		}
	} else {
		if ((chptr->mode.mode & MODE_AUDITORIUM) && !is_chanownprotop(sptr, chptr)) {
			if (!(chptr->mode.extmode & EXTMODE_NOQUIT)) {
				if (!comment) {
					sendto_chanops_butone(NULL, chptr, ":%s!%s@%s PART %s", sptr->name, sptr->user->username, GetHost(sptr), chptr->chname);
				} else {
					sendto_chanops_butone(NULL, chptr, ":%s!%s@%s PART %s :Part: %s", sptr->name, sptr->user->username, GetHost(sptr), chptr->chname, comment);
				}
			} else {
				if (!quit) {
					if (!comment) {
						...
						sendto_chanops_butone(NULL, chptr, ":%s!%s@%s PART %s :Part: +P", sptr->name, sptr->user->username, GetHost(sptr), chptr->chname);
					}
				} else {
					sendto_chanops_butone(NULL, chptr, ":%s!%s@%s PART %s :Quit: +P", sptr->name, sptr->user->username, GetHost(sptr), chptr->chname);
				}
			}
		} else {
			if (!(chptr->mode.extmode & EXTMODE_NOQUIT)) {
				if (!comment) {
					sendto_channel_butserv_butone(chptr, sptr, sptr, ":%s PART %s", sptr->name, chptr->chname);
				} else {
					sendto_channel_butserv_butone(chptr, sptr, sptr, ":%s PART %s :Part: %s", sptr->name, chptr->chname, comment);
				}
			} else {
				if (!quit) {
					if (!comment) {
						...
						sendto_channel_butserv_butone(chptr, sptr, sptr, ":%s PART %s :Part: +P", sptr->name, chptr->chname);
					}
				} else {
					sendto_channel_butserv_butone(chptr, sptr, sptr, ":%s PART %s :Quit: +P", sptr->name, chptr->chname);
				}
			}
		}
	}
