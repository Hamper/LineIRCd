...
int	w_whois(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
	...
	for (tmp = parv[1]; (nick = strtoken(&p, tmp, ",")); tmp = NULL)
	{
		    ...
		    acptr = acptr->next)
		{
			...
			{
				sendto_one(sptr, ":IRC PRIVMSG %s :%s is a Mobile User",
					sptr->name, name);
			}
			...
			if (MyConnect(acptr)) {
				if (acptr->codepage == 0) {
					sendto_one(sptr, ":IRC PRIVMSG %s :%s using codepage 'CP1251'", sptr->name, acptr->name);
				} else if (acptr->codepage == 1) {
					sendto_one(sptr, ":IRC PRIVMSG %s :%s using codepage 'KOI8-R'", sptr->name, acptr->name);
				} else if (acptr->codepage == 2) {
					sendto_one(sptr, ":IRC PRIVMSG %s :%s using codepage 'ISO-8859-5'", sptr->name, acptr->name);
				} else if (acptr->codepage == 3) {
					sendto_one(sptr, ":IRC PRIVMSG %s :%s using codepage 'UTF-8'", sptr->name, acptr->name);
				} else if (acptr->codepage == 4) {
					sendto_one(sptr, ":IRC PRIVMSG %s :%s using codepage 'IBM866'", sptr->name, acptr->name);
				}
			}
