...
add_con_refuse:
		...
		if ((bconf = Find_ban(acptr, Inet_ia2p(&acptr->ip), CONF_BAN_IP))) {
			if (bconf)
			{
				ircsprintf(zlinebuf,
					...
					"this server: %s. %s %s for more information.)\r\n",
					Inet_ia2p(&acptr->ip),
	...
	acptr->fd = fd;
	if (cptr->umodes & LISTENER_MOBILE) {
		acptr->flags |= FLAGS_MOBILE;
		if (!(cptr->umodes & LISTENER_SSL)) {
			FDwrite(fd, REPORT_MOBILE, R_mobile);
		}
	}
	if (cptr->umodes & LISTENER_CP_WIN) {
		acptr->codepage = 0;
		if (!(cptr->umodes & LISTENER_SSL)) {
			FDwrite(fd, REPORT_CP_WIN, R_cp_win);
		}
	} else if (cptr->umodes & LISTENER_CP_KOI) {
		acptr->codepage = 1;
		if (!(cptr->umodes & LISTENER_SSL)) {
			FDwrite(fd, REPORT_CP_KOI, R_cp_koi);
		}
	} else if (cptr->umodes & LISTENER_CP_MAC) {
		acptr->codepage = 2;
		if (!(cptr->umodes & LISTENER_SSL)) {
			FDwrite(fd, REPORT_CP_MAC, R_cp_mac);
		}
	} else if (cptr->umodes & LISTENER_CP_UTF) {
		acptr->codepage = 3;
		if (!(cptr->umodes & LISTENER_SSL)) {
			FDwrite(fd, REPORT_CP_UTF, R_cp_utf);
		}
	} else if (cptr->umodes & LISTENER_CP_DOS) {
		acptr->codepage = 4;
		if (!(cptr->umodes & LISTENER_SSL)) {
			FDwrite(fd, REPORT_CP_DOS, R_cp_dos);
		}
	} else {
		acptr->codepage = 0;
		if (!(cptr->umodes & LISTENER_SSL)) {
...
void	start_of_normal_client_handshake(aClient *acptr)
{
	...
	if (SHOWCONNECTINFO && !ListenerMobile(acptr) && !acptr->serv) {
...
static int read_packet(aClient *cptr, fd_set *rfd)
{
	    ...
	    !(IsPerson(cptr) && DBufLength(&cptr->recvQ) > 6090))
	{
		...
		if (cptr->flags & FLAGS_SSL)
	    		length = my_ircd_SSL_read(cptr, readbuf, sizeof(readbuf));
		else
#endif
			length = my_recv(cptr, readbuf, sizeof(readbuf), 0);
		cptr->lasttime = now;
	...
	if (check_traffic_limit(cptr, 0, length)) {
		return exit_client(cptr, cptr, cptr, "Traffic limit exceed (in)");
	}
	...
	else
	{
		...
		while (DBufLength(&cptr->recvQ) && !NoNewLine(cptr) &&
		    ((cptr->status < STAT_UNKNOWN) || ((cptr->since - now < 10) && !ClientFrozen(cptr))))
		{
			/*
	...
	do {
		...
		case ASYNC_CLIENT :
			if ((cptr = ln.value.cptr))
				...
				if (SHOWCONNECTINFO && !ListenerMobile(cptr) && !cptr->serv)
...
// Если send == 1, то проверять исходящий трафик, иначе - входящий
int check_traffic_limit (aClient *cptr, const int send, const unsigned long len) {
	traffic_counter *tc;
	int maxbps;
	time_t t, interval_time;
	...
	if (send) {
		tc = &(cptr->tc_send);
		maxbps = cptr->class->sendmaxbps;
	} else {
		tc = &(cptr->tc_recv);
		maxbps = cptr->class->recvmaxbps;
	}
	...
	if (cptr->block_transfer) {
		return 1;
	}

	t = time(NULL);
	interval_time = t - tc->bytes_prev_time;

	// Проверка на превышение
	if (interval_time > 20) {
		// Вычисление скорости передачи [б/с] за последний период
		int speed_bs = ((tc->bytes_total + len) - tc->bytes_prev) / interval_time;
		if (maxbps && (speed_bs > maxbps)) {
			// Превышение лимита
			tc->exceed_time[0] = tc->exceed_time[1];
			tc->exceed_time[1] = tc->exceed_time[2];
			tc->exceed_time[2] = t;
			if (tc->exceed_time[0] && ((tc->exceed_time[2] - tc->exceed_time[0]) < 300)) {
				// За последние 5 минут лимит превышен более 3 раз
				cptr->block_transfer = 1;
				if (send) {
					if (IsServer(cptr)) {
						ircd_log(LOG_SERVER, "Traffic limit (out) for %s is exceed (last speed: %d bytes/s)",
							cptr->name, speed_bs);
					}
					sendto_realops("Traffic limit (out) for %s is exceed (last speed: %d bytes/s)",
						cptr->name ? cptr->name : "UNKNOWN", speed_bs);
				} else {
					if (IsServer(cptr)) {
						ircd_log(LOG_SERVER, "Traffic limit (in) for %s is exceed (last speed: %d bytes/s)",
							cptr->name, speed_bs);
					}
					sendto_realops("Traffic limit (in) for %s is exceed (last speed: %d bytes/s)",
						cptr->name ? cptr->name : "UNKNOWN", speed_bs);
				}
				return 1;
			}
		}
		tc->bytes_prev = tc->bytes_total;
		tc->bytes_prev_time = t;
	}
...
/* Port encoding system */

#define TRMSG_SIZE 32768
char trmsg[TRMSG_SIZE+1];

int chcodepage(char *msg, int sfrom, int sto, unsigned int len) {
#include "codepage.h"
#define CHAR_NOT_IN_TABLE '?'
	unsigned int sym;
	int i;
	if (sfrom == sto) {
		return len;
	...
	if (sfrom == 0) {
		if (sto == 1) {
			...
			/* WIN -> UTF */
			char newmsg[TRMSG_SIZE+1];
			char *msg_ptr = msg;
			char *newmsg_ptr = newmsg;
			...
			for (sym = 0; sym < len; sym++) {
				...
				if (*msg_ptr < 128) {
					*newmsg_ptr = *msg_ptr;
					newmsg_ptr++;
					msg_ptr++;
					// Сразу выполняется continue для основного цикла,
					// поэтому переменная replaced не нужна
					continue;
				}
				// Поиск среди двухбайтных символов
				for (i=U2_COUNT-1; i >= 0; i--) {
					if (*msg_ptr == U2_WIN[i]) {
						*newmsg_ptr = U2_UTF[2*i];
						*(newmsg_ptr+1) = U2_UTF[2*i+1];
						newmsg_ptr += 2;
						msg_ptr++;
						replaced = 1;
						break;
				...
				// Поиск среди трехбайтных символов
				for (i=U3_COUNT-1; i >= 0; i--) {
					if (*msg_ptr == U3_WIN[i]) {
						*newmsg_ptr = U3_UTF[3*i];
						*(newmsg_ptr+1) = U3_UTF[3*i+1];
						*(newmsg_ptr+2) = U3_UTF[3*i+2];
						newmsg_ptr += 3;
						msg_ptr++;
						replaced = 1;
						break;
				...
				// Нет такого символа в таблицах
				*newmsg_ptr = CHAR_NOT_IN_TABLE;
				newmsg_ptr++;
				msg_ptr++;
			}
			reallen = newmsg_ptr - newmsg;
			strncpy(msg, newmsg, reallen);
			return reallen;
		}
	} else if (sto == 0) {
		if (sfrom == 1) {
			...
			while (sym < len) {
				...
				if (*msg_ptr < 128) {
					*newmsg_ptr = *msg_ptr;
					...
					sym++;
					replaced = 1;
				} else if ((*msg_ptr >= 192) && (*msg_ptr < 224)) {
					// 2 байта
					if (len - sym > 1) {
						for (i=U2_COUNT-1; i >= 0; i--) {
							if ((*msg_ptr == U2_UTF[2*i]) && (*(msg_ptr+1) == U2_UTF[2*i+1])) {
								*newmsg_ptr = U2_WIN[i];
								msg_ptr += 2;
								sym += 2;
								replaced = 1;
				...
				} else if ((*msg_ptr >= 224) && (*msg_ptr < 240)) {
					// 3 байта
					if (len - sym > 2) {
						for (i=U3_COUNT-1; i >= 0; i--) {
							if ((*msg_ptr == U3_UTF[3*i]) && (*(msg_ptr+1) == U3_UTF[3*i+1]) && (*(msg_ptr+2) == U3_UTF[3*i+2])) {
								*newmsg_ptr = U3_WIN[i];
								msg_ptr += 3;
								sym += 3;
								replaced = 1;
...
// Сообщение может стать длиннее, если нужно переводить в UTF-8
int my_send(aClient *cptr, char *msg, unsigned int len, int flags) {
	if (check_traffic_limit(cptr, 1, len)) {
		return -1;
	}
	if (!IsZipped(cptr)) {
		// Переполнения быть не должно, но на всякий случай проверка длины
		if (len < TRMSG_SIZE/3) {
			int newlen;
			...
			return(send(cptr->fd, msg, len, flags));
		}
	} else {
		return(send(cptr->fd, msg, len, flags));
	}
...
#ifdef USE_SSL
int my_ircd_SSL_write(aClient *cptr, char *msg, unsigned int len) {
	if (check_traffic_limit(cptr, 1, len)) {
		return -1;
	}
	if (!IsZipped(cptr)) {
		...
		if (len < TRMSG_SIZE/3) {
			...
			return(SSL_write((SSL *)cptr->ssl, msg, len));
		}
	} else {
		return(SSL_write((SSL *)cptr->ssl, msg, len));
	}
...
// Переводим в кодировку WINDOWS-1251, поэтому сообщение может остаться
// таким же или стать короче (если было в UTF-8)
int my_recv(aClient *cptr, char *msg, unsigned int len, int flags) {
	int rlen;
	rlen = recv(cptr->fd, msg, len, flags);
	if (rlen > 0) {
		// Переполнения быть не должно, но на всякий случай проверка длины
		if (rlen < TRMSG_SIZE) {
			rlen = chcodepage(msg, cptr->codepage, 0, rlen);
		} else {
			ircd_log(LOG_ERROR, "[Debug] (my_recv) rlen: %d; TRMSG_SIZE: %d; msg: %s", rlen, TRMSG_SIZE, msg);
		}
	}
	return(rlen);
}
...
#ifdef USE_SSL
int my_ircd_SSL_read(aClient *cptr, char *msg, unsigned int len) {
	int rlen;
	rlen = SSL_read((SSL *)cptr->ssl, msg, len);
	if (rlen > 0) {
		// Переполнения быть не должно, но на всякий случай проверка длины
		if (rlen < TRMSG_SIZE) {
			rlen = chcodepage(msg, cptr->codepage, 0, rlen);
		} else {
			ircd_log(LOG_ERROR, "[Debug] (my_ircd_SSL_read) rlen: %d; TRMSG_SIZE: %d; msg: %s", rlen, TRMSG_SIZE, msg);
		}
	}
	return(rlen);
}
