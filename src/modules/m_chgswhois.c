...
static int m_chgswhois(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
	...
	if (IsParam(2))
	{
		...
		ircd_log(LOG_CHGCMDS,
			...
			acptr->user->realhost, parv[2]);
		if (MyClient(acptr)) {
			sendto_one(acptr, ":%s NOTICE %s :*** Your SWHOIS changed to '%s'", me.name, acptr->name, parv[2]);
		}
	}
	else
	{
		...
		ircd_log(LOG_CHGCMDS,
			...
			acptr->user->realhost);
		if (MyClient(acptr)) {
			sendto_one(acptr, ":%s NOTICE %s :*** Your SWHOIS has been cleared", me.name, acptr->name);
		}
	}
...
static int m_setswhois(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
	...
	if (IsParam(1))
	{
		...
		if (!IsULine(sptr)) {
			sendto_snomask(SNO_EYES, "SWHOIS of %s (%s@%s) changed to %s", sptr->name, sptr->user->username, sptr->user->realhost, parv[1]);
			ircd_log(LOG_CHGCMDS, "SETSWHOIS: SWHOIS of %s (%s@%s) changed to %s", sptr->name, sptr->user->username, sptr->user->realhost, parv[1]);
		}
	}
	else
	{
		...
		if (!IsULine(sptr)) {
			sendto_snomask(SNO_EYES, "SWHOIS of %s (%s@%s) is now cleared", sptr->name, sptr->user->username, sptr->user->realhost);
			ircd_log(LOG_CHGCMDS, "SETSWHOIS: SWHOIS of %s (%s@%s) is now cleared", sptr->name, sptr->user->username, sptr->user->realhost);
		}
	}
