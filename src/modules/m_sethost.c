...
ModuleHeader MOD_HEADER(m_sethost)
  = {
	...
	"$Id: m_sethost.c,v 1.1.6.4 2004/07/03 19:04:38 syzop Exp $", /* Version */
...
DLLFUNC int m_sethost(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
	...
	{
		sendto_one(sptr,
			":%s NOTICE %s :*** /SetHost Error: A hostname may contain a-z, A-Z, 0-9, '_', '-', '.' & ':' - Please only use them",
		    me.name, parv[0]);
		return 0;
	}
	if (vhost[0] == ':')
	...
	if (!IsULine(sptr)) {
		sendto_snomask(SNO_EYES, "Virtual hostname of %s (%s@%s) changed to %s", sptr->name, sptr->user->username, sptr->user->realhost, vhost);
		ircd_log(LOG_CHGCMDS, "SETHOST: Virtual hostname of %s (%s@%s) changed to %s", sptr->name, sptr->user->username, sptr->user->realhost, vhost); 
	}
