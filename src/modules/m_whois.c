...
ModuleHeader MOD_HEADER(m_whois)
  = {
	...
	"$Id: m_whois.c,v 1.1.6.4 2004/07/03 19:04:44 syzop Exp $", /* Version */
...
DLLFUNC int  m_whois(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
	...
	for (tmp = parv[1]; (nick = strtoken(&p, tmp, ",")); tmp = NULL)
	{
		...
		if ((acptr = find_client(nick, NULL)))
		{
			...
			if (acptr->umodes & UMODE_MOBILE)
				sendto_one(sptr, ":%s %d %s %s :%s", me.name,
				    RPL_WHOISSPECIAL, parv[0], name, "is a Mobile User");
			...
			if (MyConnect(acptr)) {
				if (acptr->codepage == 0) {
					sendto_one(sptr, ":%s %d %s %s :%s",me.name, RPL_WHOISSPECIAL, parv[0], name, "using codepage 'CP1251'");
				} else if (acptr->codepage == 1) {
					sendto_one(sptr, ":%s %d %s %s :%s",me.name, RPL_WHOISSPECIAL, parv[0], name, "using codepage 'KOI8-R'");
				} else if (acptr->codepage == 2) {
					sendto_one(sptr, ":%s %d %s %s :%s",me.name, RPL_WHOISSPECIAL, parv[0], name, "using codepage 'ISO-8859-5'");
				} else if (acptr->codepage == 3) {
					sendto_one(sptr, ":%s %d %s %s :%s",me.name, RPL_WHOISSPECIAL, parv[0], name, "using codepage 'UTF-8'");
				} else if (acptr->codepage == 4) {
					sendto_one(sptr, ":%s %d %s %s :%s",me.name, RPL_WHOISSPECIAL, parv[0], name, "using codepage 'IBM866'");
				}
			}

			if (!BadPtr(user->swhois))
