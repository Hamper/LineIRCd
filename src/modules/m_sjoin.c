...
ModuleHeader MOD_HEADER(m_sjoin)
  = {
	"m_sjoin",
	"$Id: m_sjoin.c,v 1.1.6.2 2004/07/03 19:04:39 syzop Exp $",
	"command /sjoin", 
	"3.2-b8-1",
	NULL 
    };
...
CMD_FUNC(m_sjoin)
{
	...
	char cbuf[1024];
	char buf[1024];
	char nick[1024]; /* GCC 4 fix (backport from UnrealIRCd) */
	char *s = NULL;
	aClient *acptr;
	aChannel *chptr;
	Member *lp;
		    ...
		    && !(modeflags & CHFL_EXCEPT))
		{
			...
			if (IsMember(acptr, chptr)) {
				...
				if (IsTotalInvis(acptr)) {
					if (!(chptr->mode.mode & MODE_AUDITORIUM) || (modeflags & (CHFL_CHANOP|CHFL_CHANPROT|CHFL_CHANOWNER))) {
						sendto_channel_ntopers(acptr, chptr,  ":%s JOIN :%s", acptr->name, chptr->chname);
					} else {
						sendto_chanops_ntopers(acptr, chptr,  ":%s JOIN :%s", acptr->name, chptr->chname);
					}
				} else {
					if (chptr->mode.mode & MODE_AUDITORIUM) {
						if (modeflags & (CHFL_CHANOP|CHFL_CHANPROT|CHFL_CHANOWNER)) {
							...
							sendto_chanops_butone(NULL, chptr, ":%s!%s@%s JOIN :%s", acptr->name, acptr->user->username, GetHost(acptr), chptr->chname);
						}
					} else {
						sendto_channel_butserv(chptr, acptr, ":%s JOIN :%s", nick, chptr->chname);
					}
