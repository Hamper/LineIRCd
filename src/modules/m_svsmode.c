...
ModuleHeader MOD_HEADER(m_svsmode)
  = {
	"m_svsmode",
	"$Id: m_svsmode.c,v 1.1.6.4 2004/07/03 19:04:40 syzop Exp $",
...
char *xtok = show_change ? TOK_SVS2MODE : TOK_SVSMODE;
	...
	for (m = parv[2]; *m; m++)
		switch (*m)
		{
			...
			case 'I':
				if ((what == MODE_ADD) && !(acptr->umodes & UMODE_TOTALINVIS)) {
					sendto_channels_inviso_part(acptr);
				}
				if ((what == MODE_DEL) && (acptr->umodes & UMODE_TOTALINVIS)) {
