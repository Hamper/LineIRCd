...
ModuleHeader MOD_HEADER(m_setident)
  = {
	...
	"$Id: m_setident.c,v 1.1.6.4 2004/07/03 19:04:38 syzop Exp $", /* Version */
...
DLLFUNC int m_setident(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
	...
	if (!IsULine(sptr)) {
		sendto_snomask(SNO_EYES, "Virtual ident of %s (%s@%s) changed to %s", sptr->name, sptr->user->username, sptr->user->realhost, vident);
		ircd_log(LOG_CHGCMDS, "SETIDENT: Virtual ident of %s (%s@%s) changed to %s", sptr->name, sptr->user->username, sptr->user->realhost, vident);
	}
