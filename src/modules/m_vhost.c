...
ModuleHeader MOD_HEADER(m_vhost)
  = {
	...
	"$Id: m_vhost.c,v 1.1.6.4 2004/07/03 19:04:43 syzop Exp $", /* Version */
...
int  m_vhost(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
	...
	if (!stricmp(ircnet005, "IRCLine.RU")) {
		sendto_one(sptr, ":%s NOTICE %s :*** /vhost is disabled on %s", me.name, sptr->name, ircnet005);
		return 0;
	}
