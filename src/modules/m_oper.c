...
static oper_oflag_t oper_oflags[7]; 
...
static void init_operflags()
{
	...
	oper_oflags[3].oflag = OFLAG_ISGLOBAL;
	oper_oflags[3].umode = &UMODE_OPER;
	oper_oflags[3].host = &oper_host;
	oper_oflags[3].announce = "is now an operator (O)";
	oper_oflags[4].oflag = OFLAG_HELPOP;
	oper_oflags[4].umode = &UMODE_HELPOP;
	oper_oflags[4].host = NULL;
	oper_oflags[4].announce = NULL;
	oper_oflags[5].oflag = OFLAG_WHOIS;
	oper_oflags[5].umode = &UMODE_WHOIS;
	oper_oflags[5].host = NULL;
	oper_oflags[5].announce = NULL;
	oper_oflags[6].oflag = 0;
	oper_oflags[6].umode = NULL;
...
ModuleHeader MOD_HEADER(m_oper)
  = {
	"oper",	/* Name of module */
	"$Id: m_oper.c,v 1.1.6.4 2004/07/03 19:04:36 syzop Exp $", /* Version */
...
#ifdef ONLY_SERVOP_CAN_GLOBAL
	if ((aconf->oflags & OFLAG_ISGLOBAL) && find_server_quick(SERVICES_NAME) && !IsServOp(sptr)) {
		sendto_one(sptr, err_str(ERR_NOOPERHOST), me.name, parv[0]);
		sendto_one(sptr, ":%s NOTICE %s :You must identify as a Services Operator before you can get a Global IRC Operator status.", me.name, sptr->name);
		sendto_realops("Failed OPER attempt by %s (%s@%s) [not identified on services]", parv[0], sptr->user->username, sptr->sockhost);
		sendto_serv_butone(&me, ":%s GLOBOPS :Failed OPER attempt by %s (%s@%s) [not identified on services]", me.name, parv[0], sptr->user->username, sptr->sockhost);
		ircd_log(LOG_OPER, "OPER NOTSERVOP (%s) by (%s!%s@%s)", name, parv[0], sptr->user->username, sptr->sockhost);
		sptr->since += 7;
		return 0;
	}
#endif
...
/* handle oflags that trigger umodes */
		...
		sendto_one(sptr, ":%s NOTICE %s :*** Your OFLAGS have been set to: %s", me.name, sptr->name, oflagstr(sptr->oflag));
	...
	if (i == -1)
	{
		...
		sendto_serv_butone(&me,
		    ":%s GLOBOPS :Failed OPER attempt by %s (%s@%s) using UID %s [FAILEDAUTH]",
