...
ModuleHeader MOD_HEADER(m_kick)
  = {
	"m_kick",
	"$Id: m_kick.c,v 1.1.6.2 2004/07/03 19:04:35 syzop Exp $",
	"command /kick", 
	"3.2-b8-1",
	NULL 
    };
	...
	for (; (name = strtoken(&p, parv[1], ",")); parv[1] = NULL)
	{
		...
		for (; (user = strtoken(&p2, parv[2], ",")); parv[2] = NULL)
		{
			...
			if ((lp = find_membership_link(who->user->channel, chptr)))
			{
				...
				if (IsServices(who)) {
#endif
					sendto_one(sptr, ":%s NOTICE %s :*** Cannot kick %s from %s (usermode +S)", me.name, sptr->name, who->name, chptr->chname);
					goto deny;
				}
				...
				if (lp)
				{
					    ...
					    !(lp->flags & (CHFL_CHANOP|CHFL_CHANPROT|CHFL_CHANOWNER)))
					{
						...
						if (MyClient(who))
							sendto_prefix_one(who, sptr, 1, ":%s KICK %s %s :%s",
								sptr->name, chptr->chname, who->name, comment);
					} else {
						/* NORMAL */
