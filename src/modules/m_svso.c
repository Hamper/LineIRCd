...
ModuleHeader MOD_HEADER(m_svso)
  = {
	"m_svso",
	"$Id: m_svso.c,v 1.1.6.4 2004/07/03 19:04:41 syzop Exp $",
	"command /svso", 
	"3.2-b8-1",
	NULL 
    };
...
int m_svso(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
        ...
        if (*parv[2] == '+')
        {
				...
				sendto_one(acptr, ":%s NOTICE %s :*** Services changed your OFLAGS to: %s", me.name, acptr->name, oflagstr(acptr->oflag));
        }
        if (*parv[2] == '-')
        {
                fLag = acptr->umodes;
		...
		sendto_one(acptr, ":%s NOTICE %s :*** Services have cleared your OFLAGS", me.name, acptr->name);
