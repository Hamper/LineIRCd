...
DLLFUNC int m_chghost(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
	...
	{
		sendto_one(sptr,
		    ":%s NOTICE %s :*** /ChgHost Error: A hostname may contain a-z, A-Z, 0-9, '_', '-', '.' & ':' - Please only use them",
		    me.name, parv[0]);
		return 0;
	}
	...
	if ((acptr = find_person(parv[1], NULL)))
	{
		...
		if (MyClient(acptr) && !IsULine(sptr)) {
			sendto_one(acptr, ":%s NOTICE %s :*** Your virtual hostname changed to %s", me.name, acptr->name, parv[2]);
		}
		return 0;
	}
	else
