...
ModuleHeader MOD_HEADER(m_stats)
  = {
	"m_stats",
	"$Id: m_stats.c,v 1.1.6.2 2004/07/03 19:04:39 syzop Exp $",
	"command /stats", 
	"3.2-b8-1",
	NULL 
    };
...
DLLFUNC CMD_FUNC(m_stats)
{
	...
	if (stat)
	{
		...
		sendto_snomask(SNO_EYES, "Stats \'%c\' requested by %s (%s@%s)",
			stat->flag, sptr->name, sptr->user->username, sptr->user->realhost);
	}
	else
	{
		stats_help(sptr);
...
static char buf[256];
	...
	if (listener->umodes & LISTENER_SSL)
		strcat(buf, "SSL ");
	if (listener->umodes & LISTENER_MOBILE)
		strcat(buf, "mobile ");
	if (listener->umodes & LISTENER_CP_WIN)
		strcat(buf, "CP1251 ");
	if (listener->umodes & LISTENER_CP_KOI)
		strcat(buf, "KOI8-R ");
	if (listener->umodes & LISTENER_CP_MAC)
		strcat(buf, "ISO-8859-5 ");
	if (listener->umodes & LISTENER_CP_UTF)
		strcat(buf, "UTF-8 ");
	if (listener->umodes & LISTENER_CP_DOS)
		strcat(buf, "IBM866 ");
	return buf;
}
...
int stats_set(aClient *sptr, char *para)
{
	...
	sendto_one(sptr, ":%s %i %s :local-postfix: %s", me.name, RPL_TEXT,
	    sptr->name, local_postfix);
	sendto_one(sptr, ":%s %i %s :local-postfix-for-all: %d", me.name, RPL_TEXT,
	    sptr->name, local_postfix_for_all);
	sendto_one(sptr, ":%s %i %s :no-hide-local: %d", me.name, RPL_TEXT,
	    sptr->name, no_hide_local);
	...
	*modebuf = *parabuf = 0;
	...
	sendto_one(sptr, ":%s %i %s :accept-limit: %d", me.name, RPL_TEXT,
		sptr->name, ACCEPT_LIMIT);
	sendto_one(sptr, ":%s %i %s :callerid-time: %d", me.name, RPL_TEXT,
		sptr->name, CALLERID_TIME);
...
int stats_class(aClient *sptr, char *para)
{
	...
	for (classes = conf_class; classes; classes = (ConfigItem_class *) classes->next) 
	{
		sendto_one(sptr, rpl_str(RPL_STATSYLINE),
			...
			classes->sendmaxbps, classes->recvmaxbps, classes->minlastseen);
#ifdef DEBUGMODE
