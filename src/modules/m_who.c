...
/* $Id: m_who.c,v 1.1.6.4 2004/07/03 19:04:43 syzop Exp $ */
...
ModuleHeader MOD_HEADER(m_who)
  = {
	"who",	/* Name of module */
	"$Id: m_who.c,v 1.1.6.4 2004/07/03 19:04:43 syzop Exp $", /* Version */
		...
		switch (*s)
		{
			...
			case 'm':
				REQUIRE_PARAM()
				{
					...
					if (!IsAnOper(sptr))
						*umodes = *umodes & (UMODE_OPER | UMODE_LOCOP | UMODE_SADMIN | UMODE_ADMIN | UMODE_NETADMIN);
					if (*umodes == 0)
						return -1;
				}
				i++;
	...
	*flg = 0;
	...
	int cansee = ShowChannel(sptr, chptr);
		...
		if (cansee && (acptr->umodes & UMODE_HIDEWHOIS) && !IsMember(sptr, chptr))
			cansee = 0;
		if (cansee && IsTotalInvis(acptr) && (acptr != sptr) && !IsAnOper(sptr))
			cansee = 0;
		if (!cansee)
		{
			if (IsAnOper(sptr))
