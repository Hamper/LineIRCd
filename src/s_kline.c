...
int  find_tkline_match(aClient *cptr, int xx)
{
	...
	if ((lp->type & TKL_KILL) && !(lp->type & TKL_GLOBAL)) {
		// K:Line
		if (ClientExceptK(cptr)) {
			return 1;
		}
	}
	if ((lp->type & TKL_KILL) && (lp->type & TKL_GLOBAL)) {
		// G:Line
		if (ClientExceptG(cptr)) {
			return 1;
		}
	}
	...
	if ((lp->type & TKL_KILL) && (xx != 2))
	{
		if (lp->type & TKL_GLOBAL)
		{
			ircstp->is_ref++;
			sendto_one(cptr,
				":%s NOTICE %s :*** You are banned from %s (%s) Email\\visit %s for more information.",
					me.name, cptr->name,
					ircnetwork, lp->reason, GLINE_ADDRESS);
			ircsprintf(msge, "User has been banned from %s (%s)",
		...
		else
		{
			...
			ircsprintf(msge, "User is banned (%s)",
				lp->reason);
			return (exit_client(cptr, cptr, &me,
				msge));
	...
	if (lp->type & TKL_ZAP)
	{
		ircstp->is_ref++;
		if (lp->type & TKL_GLOBAL) {
			ircsprintf(msge, "Z:lined (%s) Email\\visit %s for more information.", lp->reason, GLINE_ADDRESS);
		} else {
			ircsprintf(msge, "Z:lined (%s) Email\\visit %s for more information.", lp->reason, KLINE_ADDRESS);
		}
...
int is_noident(aClient *cptr) {
	return 0;
#if 0
	aTKline *lp;
	for (lp = tklines[tkl_hash('m')]; lp; lp = lp->next) {
		if (!(lp->type & TKL_NOIDENT)) {
			continue;
		}

		// Сравнение только по IP, т.к. хост при вызове данной функции
		// ещё не определен.
		...
		if (lp->ptr.netmask) {
			...
			if (!match(lp->hostmask, GetIP(cptr))) {
				return 1;
			}
		}
	}
...
int m_tkl(aClient *cptr, aClient *sptr, int parc, char *parv[])
{
	...
	switch (*parv[1])
	{
	  case '+':
	  {
		...
		if ((type & TKL_SPAMF) && unreal_checkregex(reason,0,1)) {
			ircd_log(LOG_ERROR, "[PROTOCOL ERROR] Error in regex (spamfilter from %s): %s", sptr->name, reason);
			sendto_snomask(SNO_JUNK, "Error in regex (spamfilter from %s): %s", sptr->name, reason);
			return 0;
		}
...
char *str = (char *)StripControlCodes(str_in);
	...
	for (tk = tklines[tkl_hash('F')]; tk; tk = tk->next)
	{
		if (!(tk->subtype & type))
			continue;
		if (!regexec(&tk->ptr.spamf->expr, my_str_convert(str), 0, NULL, 0))
		{
			/* matched! */
			char buf[1024];
			char targetbuf[48];
