...
static OperFlag _ListenerFlags[] = {
	...
	{ LISTENER_CP_DOS,	"cp_dos"},
	{ LISTENER_CP_KOI,	"cp_koi"},
	{ LISTENER_CP_MAC,	"cp_mac"},
	{ LISTENER_CP_UTF,	"cp_utf"},
	{ LISTENER_CP_WIN,	"cp_win"},
	{ LISTENER_MOBILE,	"mobile"},
...
void	free_iConf(aConfiguration *i)
{
	...
	ircfree(i->network.x_local_postfix);
...
#define Error(x) { config_error((x)); errors++; }
	...
	if (!requiredstuff.settings.gline_address)
		Error("set::gline-address missing");
	...
	for(tld = conf_tld; tld; tld = (ConfigItem_tld *) tld->next)
	{
		if (!match(tld->mask, uhost))
		{
			...
			if ((tld->options & TLD_REMOTE) && MyClient(cptr))
				continue;
			if ((tld->options & TLD_MOBILE) && !IsMobile(cptr))
				continue;
			return tld;
		}
	}
...
ConfigItem_ban 	*Find_ban(aClient *sptr, char *host, short type)
{
	...
	for (ban = conf_ban; ban; ban = (ConfigItem_ban *) ban->next)
	{
		if (ban->flag.type == type)
		{
			if (sptr)
			{
				if ((type == CONF_BAN_USER) && ClientExceptK(sptr)) {
					return NULL;
				}
...
int	AllowClient(aClient *cptr, struct hostent *hp, char *sockhost, char *username)
{
	...
	for (aconf = conf_allow; aconf; aconf = (ConfigItem_allow *) aconf->next)
	{
		...
		if (aconf->flags.ssl && !IsSecure(cptr))
			continue;
		if (aconf->flags.mobile && !ListenerMobile(cptr))
			continue;
		if (hp)
...
/*		if (index(uhost, '@'))  now flag based -- codemastr */
		...
		if (local_postfix && (!hp || local_postfix_for_all)) {
			if (local_ipv4(cptr->user->ip_str)) {
				unsigned int a, b, c, d;
				sscanf(cptr->user->ip_str, "%u.%u.%u.%u", &a, &b, &c, &d);
				ircsprintf(uhost,  "%d-%d-%d-%d.%s", a, b, c, d, local_postfix);
				cptr->flags |= FLAGS_LPOSTFIX;
			}
		}
...
int	_test_me(ConfigFile *conf, ConfigEntry *ce)
{
	...
	else
	{
		if (cep->ce_vardata)
		{
			...
			{
				config_error("%s:%i: illegal me::name contains invalid character(s) [only a-z, A-Z, 0-9, '_', '-', '.' & ':' are allowed]",
					cep->ce_fileptr->cf_filename, cep->ce_varlinenum);
				errors++;
			}
		}
...
int	_conf_oper(ConfigFile *conf, ConfigEntry *ce)
{
	...
	if ((cep = config_find_entry(ce->ce_entries, "level")))
	{
		if (stricmp(cep->ce_vardata, "global") == 0) {
			/* Global IRC Operator
			 * Flags: -O----r--hwgcLkKbBnGztZW-vq---fm
			 * Flags: global, can_rehash, helpop, can_wallops, can_globops,
			 *        can_localroute, can_globalroute, can_localkill, can_globalkill,
			 *        can_kline, can_unkline, can_localnotice, can_globalnotice,
			 *        can_zline, can_gkline, can_gzline, get_umodew, can_override, can_setq
			 *        can_nofakelag, nochanlimit
			 */
			oper->oflags |= (OFLAG_GLOBAL|OFLAG_REHASH|OFLAG_HELPOP|OFLAG_WALLOP|OFLAG_GLOBOP|OFLAG_LROUTE|OFLAG_GROUTE|OFLAG_LKILL|OFLAG_GKILL|OFLAG_KLINE|OFLAG_UNKLINE|OFLAG_LNOTICE|OFLAG_GNOTICE|OFLAG_ZLINE|OFLAG_TKL|OFLAG_GZL|OFLAG_WHOIS|OFLAG_OVERRIDE|OFLAG_UMODEQ|OFLAG_NOFAKELAG|OFLAG_NOCHANLIMIT);
		} else if (stricmp(cep->ce_vardata, "admin") == 0) {
			/* Server Administrator
			 * Flags: -OA--r--hwgcLkKbBnGztZW-vq-d-fm
			 * Flags: global, admin, can_rehash, helpop, can_wallops, can_globops,
			 *        can_localroute, can_globalroute, can_localkill, can_globalkill,
			 *        can_kline, can_unkline, can_localnotice, can_globalnotice,
			 *        can_zline, can_gkline, can_gzline, get_umodew, can_override, can_setq,
			 *        can_dccdeny, can_nofakelag, nochanlimit
			 */
			oper->oflags |= (OFLAG_GLOBAL|OFLAG_ADMIN|OFLAG_REHASH|OFLAG_HELPOP|OFLAG_WALLOP|OFLAG_GLOBOP|OFLAG_LROUTE|OFLAG_GROUTE|OFLAG_LKILL|OFLAG_GKILL|OFLAG_KLINE|OFLAG_UNKLINE|OFLAG_LNOTICE|OFLAG_GNOTICE|OFLAG_ZLINE|OFLAG_TKL|OFLAG_GZL|OFLAG_WHOIS|OFLAG_OVERRIDE|OFLAG_UMODEQ|OFLAG_DCCDENY|OFLAG_NOFAKELAG|OFLAG_NOCHANLIMIT);
		} else if (stricmp(cep->ce_vardata, "sadmin") == 0) {
			/* Services Administrator
			 * Flags: -OAa-r--hwgcLkKbBnGztZW-vq-d-fm
			 * Flags: global, admin, services-admin, can_rehash, helpop, can_wallops,
			 *        can_globops, can_localroute, can_globalroute, can_localkill,
			 *        can_globalkill, can_kline, can_unkline, can_localnotice,
			 *        can_globalnotice, can_zline, can_gkline, can_gzline, get_umodew,
			 *        can_override, can_setq, can_dccdeny, can_nofakelag, nochanlimit
			 */
			oper->oflags |= (OFLAG_GLOBAL|OFLAG_ADMIN|OFLAG_SADMIN_|OFLAG_REHASH|OFLAG_HELPOP|OFLAG_WALLOP|OFLAG_GLOBOP|OFLAG_LROUTE|OFLAG_GROUTE|OFLAG_LKILL|OFLAG_GKILL|OFLAG_KLINE|OFLAG_UNKLINE|OFLAG_LNOTICE|OFLAG_GNOTICE|OFLAG_ZLINE|OFLAG_TKL|OFLAG_GZL|OFLAG_WHOIS|OFLAG_OVERRIDE|OFLAG_UMODEQ|OFLAG_DCCDENY|OFLAG_NOFAKELAG|OFLAG_NOCHANLIMIT);
		} else if (stricmp(cep->ce_vardata, "netadmin") == 0) {
			/* Network Administrator
			 * Flags: -OAaNr--hwgcLkKbBnGztZW-vq-d-fm
			 * Flags: global, admin, services-admin, netadmin, can_rehash, helpop,
			 *        can_wallops, can_globops, can_localroute, can_globalroute,
			 *        can_localkill, can_globalkill, can_kline, can_unkline, can_localnotice,
			 *        can_globalnotice, can_zline, can_gkline, can_gzline, get_umodew,
			 *        can_override, can_setq, can_dccdeny, can_nofakelag, nochanlimit
			 */
			oper->oflags |= (OFLAG_GLOBAL|OFLAG_ADMIN|OFLAG_SADMIN_|OFLAG_NADMIN|OFLAG_REHASH|OFLAG_HELPOP|OFLAG_WALLOP|OFLAG_GLOBOP|OFLAG_LROUTE|OFLAG_GROUTE|OFLAG_LKILL|OFLAG_GKILL|OFLAG_KLINE|OFLAG_UNKLINE|OFLAG_LNOTICE|OFLAG_GNOTICE|OFLAG_ZLINE|OFLAG_TKL|OFLAG_GZL|OFLAG_WHOIS|OFLAG_OVERRIDE|OFLAG_UMODEQ|OFLAG_DCCDENY|OFLAG_NOFAKELAG|OFLAG_NOCHANLIMIT);
		} else {
			if (stricmp(cep->ce_vardata, "local") != 0) {
				config_status("%s:%i: illegal oper::level, unknown level '%s' using 'local'",
						cep->ce_fileptr->cf_filename,
						cep->ce_varlinenum,
						cep->ce_vardata);
			}
			/* Local IRC Operator
			 * Flags: o----r--h-gc-k-bBn-z--W------fm
			 * Flags: local, can_rehash, helpop, can_globops,
			 *        can_localroute, can_localkill, can_kline, can_unkline,
			 *        can_localnotice, can_zline, get_umodew, can_nofakelag, nochanlimit
			 */
			oper->oflags |= (OFLAG_LOCAL|OFLAG_REHASH|OFLAG_HELPOP|OFLAG_GLOBOP|OFLAG_LROUTE|OFLAG_LKILL|OFLAG_KLINE|OFLAG_UNKLINE|OFLAG_LNOTICE|OFLAG_ZLINE|OFLAG_WHOIS|OFLAG_NOFAKELAG|OFLAG_NOCHANLIMIT);
		}
	}
...
int	_test_oper(ConfigFile *conf, ConfigEntry *ce)
{
	...
	for (cep = ce->ce_entries; cep; cep = cep->ce_next)
	{
		...
		if (!cep->ce_entries)
		{
			...
			else if (!strcmp(cep->ce_varname, "level")) {
			}
	...
	if (!config_find_entry(ce->ce_entries, "flags") && !config_find_entry(ce->ce_entries, "level"))
	{
...
int	_conf_class(ConfigFile *conf, ConfigEntry *ce)
{
	...
	if ((cep = config_find_entry(ce->ce_entries, "sendmaxbps")))
	{
		class->sendmaxbps = atol(cep->ce_vardata);
	}
	if ((cep = config_find_entry(ce->ce_entries, "recvmaxbps")))
	{
		class->recvmaxbps = atol(cep->ce_vardata);
	}
	if ((cep = config_find_entry(ce->ce_entries, "connfreq")))
	{
		class->connfreq = atol(cep->ce_vardata);
	}
	if ((cep = config_find_entry(ce->ce_entries, "minlastseen")))
	{
		class->minlastseen = atol(cep->ce_vardata);
	}
	if (isnew) 
...
int	_test_class(ConfigFile *conf, ConfigEntry *ce)
{
	...
	for (cep = ce->ce_entries; cep; cep = cep->ce_next)
	{
		...
		else if (!strcmp(cep->ce_varname, "minlastseen")) {}
		else if (!strcmp(cep->ce_varname, "sendq")) {}
		else if (!strcmp(cep->ce_varname, "recvq")) {}
		else if (!strcmp(cep->ce_varname, "sendmaxbps")) {}
		else if (!strcmp(cep->ce_varname, "recvmaxbps")) {}
		else
		{
...
int     _conf_tld(ConfigFile *conf, ConfigEntry *ce)
{
	...
	if (cep)
	{
		for (cep = cep->ce_entries; cep; cep = cep->ce_next)
		{
			...
			else if (!strcmp(cep->ce_varname, "mobile"))
				ca->options |= TLD_MOBILE;
		}
	}	
        ...
        for (cep = ce->ce_entries; cep; cep = cep->ce_next)
	{
		...
		if (!strcmp(cep->ce_varname, "mask")) {
			...
			for (cep2 = cep->ce_entries; cep2; cep2 = cep2->ce_next)
			{
				...
				else if (!strcmp(cep2->ce_varname, "mobile")) {
				}
				else
				{
...
int	_conf_allow(ConfigFile *conf, ConfigEntry *ce)
{
	...
	if ((cep = config_find_entry(ce->ce_entries, "options")))
	{
		for (cepp = cep->ce_entries; cepp; cepp = cepp->ce_next) {
			...
			else if (!strcmp(cepp->ce_varname, "mobile")) 
				allow->flags.mobile = 1;
		}
...
int     _conf_badword(ConfigFile *conf, ConfigEntry *ce)
{
	...
	my_str_tolower_rus2eng(cep->ce_vardata);
#ifdef FAST_BADWORD_REPLACE
...
int	_conf_link(ConfigFile *conf, ConfigEntry *ce)
{
	...
	// Даже если сервера сейчас в сети нет - отсчёт времени начинается с
	// текущего времени
	link->lastseen = time(NULL);
...
int	_conf_set(ConfigFile *conf, ConfigEntry *ce)
{
	...
	for (cep = ce->ce_entries; cep; cep = cep->ce_next)
	{
		...
		else if (!strcmp(cep->ce_varname, "accept-limit")) {
			tempiConf.accept_limit = atol(cep->ce_vardata);
		}
		else if (!strcmp(cep->ce_varname, "callerid-time")) {
			tempiConf.callerid_time = atol(cep->ce_vardata);
		}
		...
		else if (!strcmp(cep->ce_varname, "local-postfix")) {
			ircstrdup(tempiConf.network.x_local_postfix, cep->ce_vardata);
		}
		else if (!strcmp(cep->ce_varname, "local-postfix-for-all")) {
			tempiConf.network.x_local_postfix_for_all = config_checkval(cep->ce_vardata,CFG_YESNO);
		}
		else if (!strcmp(cep->ce_varname, "no-hide-local")) {
			tempiConf.network.x_no_hide_local = config_checkval(cep->ce_vardata,CFG_YESNO);
		}
...
#define CheckNull(x) if ((!(x)->ce_vardata) || (!(*((x)->ce_vardata)))) { config_error("%s:%i: missing parameter", (x)->ce_fileptr->cf_filename, (x)->ce_varlinenum); errors++; continue; }
	for (cep = ce->ce_entries; cep; cep = cep->ce_next)
	{
		...
		if (!strcmp(cep->ce_varname, "kline-address")) {
			...
			requiredstuff.settings.gline_address = 1;
		}
		else if (!strcmp(cep->ce_varname, "modes-on-connect")) {
			CheckNull(cep);
			...
			if (strchr(cep->ce_vardata, ' ') || (*cep->ce_vardata == ':'))
			{
				config_error("%s:%i: set::local-postfix must not contain spaces or be prefixed with ':'",
					cep->ce_fileptr->cf_filename, cep->ce_varlinenum);
				errors++;
				continue;
			}
