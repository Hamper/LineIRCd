...
int register_user(aClient *cptr, aClient *sptr, char *nick, char *username, char *umode, char *virthost, char *ip)
{
	...
	if (MyConnect(sptr))
	{
		...
		// Новый servicestamp может быть задан командой IDREPLY
		// Если новый servicestamp не задан, то idreq_servicestamp == 0,
		// что соответствует значению servicestamp по-умолчанию
		user->servicestamp = sptr->idreq_servicestamp;

		// Запись username
		if (!(sptr->flags & FLAGS_DOID)) {
			// Идентификация не требуется по блоку allow
			// Использовать указанный пользователем username
			strncpyzt(user->username, username, USERLEN + 1);
		} else if (sptr->flags & FLAGS_GOTID) {
			// Получен ответ от identd
			// Использовать ответ identd
			strncpyzt(user->username, sptr->username, USERLEN + 1);
		} else {
			// Возможные варианты:
			// 1) Идентификация отключена для всех в блоке set
			// 2) Ожидался, но не был получен ответ от identd
			// 3) Пропущена идентификация из-за M:Line
			// Username may point to user->username
			char temp[USERLEN + 1];
			strncpyzt(temp, username, USERLEN + 1);
#if 1
			if (ClientSUser(sptr)) {
				// Использовать полученный от сервисов username
				strncpyzt(user->username, sptr->username, USERLEN + 1);
#else
			if (ClientNoIdent(sptr)) {
				// Пропущена идентификация из-за M:Line
				if (ClientSUser(sptr)) {
					// Использовать полученный от сервисов username
					strncpyzt(user->username, sptr->username, USERLEN + 1);
				} else {
					// Не было указания от сервисов заменить username
					// Использовать указанный пользователем, но с тильдой
					*user->username = '~';
				...
				// Идентификация отключена для всех
				// Использовать указанный пользователем username
				strncpyzt(user->username, temp, USERLEN + 1);
			} else {
				// Ожидался, но не был получен ответ от identd
				// Добавить перед указанным пользователем username тильду
				*user->username = '~';
				strncpyzt((user->username + 1), temp, USERLEN);
#ifdef HOSTILENAME
				noident = 1;
