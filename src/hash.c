...
unsigned int hash_nn_name(const char *hname)
{
	...
	for (hash_value = 0; *hname; ++hname)
	{
		...
		hash_value = (hash_value << ONE_EIGHTH) + my_tolower(*hname);
...
unsigned hash_nick_name(char *nname)
{
	...
	while (*nname)
	{
		lower = my_tolower_rus2eng(*nname);
		hash = (hash << 1) + lower;
		hash2 = (hash2 >> 1) + lower;
		nname++;
	}
...
unsigned int  hash_channel_name(char *name)
{
	...
	while (*hname && --i)
	{
		lower = my_tolower_rus2eng(*hname);
		hash = (hash << 1) + lower;
		hash2 = (hash2 >> 1) + lower;
		hname++;
	}
...
aChannel *hash_find_channel(char *name, aChannel *chptr)
{
	...
	for (tmp = (aChannel *)tmp3->list; tmp; tmp = tmp->hnextch)
		if (my_smycmp(name, tmp->chname) == 0)
		{
			return (tmp);
		}
	return chptr;
