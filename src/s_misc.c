...
// Обновление времени последнего появления сервера для всех блоков link
// с именем этого сервера (их может быть несколько)
void update_server_lastseen (aClient *cptr) {
	ConfigItem_link *aconf;
	for (aconf = conf_link; aconf; aconf = (ConfigItem_link *) aconf->next) {
		if (!match(aconf->servername, cptr->name)) {
			aconf->lastseen = time(NULL);
		}
	}
}
...
static void exit_one_client(aClient *cptr, aClient *sptr, aClient *from, char *comment, int split)
{
	...
	else if (sptr->name[0])	/* ...just clean all others with QUIT... */
	{
		...
		if (sptr->user)
		{
			Membership *channels, *next;

			if (strstr(comment, prefix_quit) == comment) {
				for (channels = sptr->user->channel; channels; channels = next) {
					next = channels->next;
					if (channels->chptr->mode.extmode & EXTMODE_NOQUIT) {
						send_part_to_local_users(sptr, channels->chptr, "+P", 1);
						remove_user_from_channel(sptr, channels->chptr);
					}
				}
				sendto_common_channels_normalquit(sptr, comment);
			} else {
				sendto_common_channels(sptr, ":%s QUIT :%s", sptr->name, comment);
			}
			...
			while ((channels = sptr->user->channel))
				remove_user_from_channel(sptr, channels->chptr);
			...
			/* Clean up acceptfield */
			while ((lp = sptr->user->accept)) {
				if (AcceptIsUser(lp)) {
					aClient *tmp;
					tmp = lp->value.cptr;
					if (!del_accept(sptr, tmp, NULL, 0)) {
						del_accept(tmp, sptr, NULL, 1);
					}
				} else if (AcceptIsMask(lp)) {
					del_accept(sptr, NULL, lp->value.cp, 0);
				} else if (AcceptIsRef(lp)) {
					aClient *tmp;
					tmp = lp->value.cptr;
					del_accept(sptr, tmp, NULL, 1);
					del_accept(tmp, sptr, NULL, 0);
				}
			}
...
int local_ipv4(char *ip) {
	unsigned int a, b, c, d;
	if (strchr(ip, ':')) { /* IPv6 is not supported */
		return 0;
	}
...
Spamfilter *e = MyMallocEx(sizeof(Spamfilter));
	...
	regcomp(&e->expr, my_str_convert(s), SPF_REGEX_FLAGS);
	return e;
}
