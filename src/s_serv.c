...
void m_info_send(aClient *sptr)
{
	...
	sendto_one(sptr, ":%s %d %s :| * nighthawk    <nighthawk@unrealircd.com>",
	    me.name, RPL_INFO, sptr->name);
	sendto_one(sptr, ":%s %d %s :|", me.name, RPL_INFO, sptr->name);
	sendto_one(sptr, ":%s %d %s :| Unreal-DalNetRU Coder:", me.name, RPL_INFO, sptr->name);
	sendto_one(sptr, ":%s %d %s :|", me.name, RPL_INFO, sptr->name);
	sendto_one(sptr, ":%s %d %s :| * JSergey      <sergey@ircd.com.ru>",
	    me.name, RPL_INFO, sptr->name);
	sendto_one(sptr, ":%s %d %s :|", me.name, RPL_INFO, sptr->name);
	...
	*p++ = '[';
	if (acptr->flags & FLAGS_LISTEN)
	{
		...
		if (acptr->umodes & LISTENER_SSL)
			*p++ = 's';
#endif
		if (acptr->umodes & LISTENER_MOBILE)
			*p++ = 'm';
		if (acptr->umodes & LISTENER_CP_WIN)
			*p++ = 'W';
		if (acptr->umodes & LISTENER_CP_KOI)
			*p++ = 'K';
		if (acptr->umodes & LISTENER_CP_MAC)
			*p++ = 'M';
		if (acptr->umodes & LISTENER_CP_UTF)
			*p++ = 'U';
		if (acptr->umodes & LISTENER_CP_DOS)
			*p++ = 'D';
	}
	else
	{
